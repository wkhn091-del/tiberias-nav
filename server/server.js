...
//   client -> server  category search  {type:"category_search", category:"food"|"gas"|"emergency", lat, lon}
...
async function loadRoute() {
  routeDoc = await Route.findOne({ slug: ROUTE_SLUG }).lean();
  if (!routeDoc) {
...
function resolveLaneSamples(probes, ways) {
  ...
  for (const [plat, plon] of probes) {
    ...
    if (bestWay && bestDist <= LANES_MATCH_M) {
      ...
      out.push({ lanes: null, highway: "", tagged: false, maxspeedKmh: null, oneway: false });
    }
  }
  return out;
}
...
async function fetchOsrmRoute(origin, destination, profile = DEFAULT_PROFILE) {
  ...
  const url = `${OSRM_BASE_URL}/${engine}/route/v1/driving/${coords}?geometries=geojson&overview=full&steps=true`;
    ...
    if (Array.isArray(coords) && coords.length > 1) {
      fetchRouteCorridor(coords)
        .then(({ signals, laneSamples, probes }) => {
          ...
          const allSignals = [...clustered, ...synthesized];
    ...
    try {
      ...
      if (m.type === "set_destination") {
        ...
        try {
          ...
          const { geometry, distanceM, durationS, steps } = await fetchOsrmRoute(origin, destination, profile);
          activateRoute(
      ...
      Ping.create({
        ...
        ts,
      }).catch((dbErr) => console.warn("ping not persisted:", dbErr.message));
...
mongoose
  ...
  .catch((err) => {
    ...
    loadRoute().catch((e) => console.warn("route load failed:", e.message));
  });
...
async function rehydrateRecentReports() {
  try {
    ...
    const docs = await Report.find({ ts: { $gte: since } }).sort({ ts: 1 }).limit(200).lean();
    for (const d of docs) {

...
/**
 * Road classes where a signalised junction is plausible.
 *
 * Excludes residential, service and unclassified: lights do exist there, but
 * rarely enough that guessing costs more trust than it earns.
 */
...
/** Class of the nearest corridor sample, or null when there is no evidence. */
function nearestSampleClass(laneSamples, lat, lon, maxDistM = 60) {
  if (!Array.isArray(laneSamples) || !laneSamples.length) return null;
  let best = null;
  let bestD = Infinity;
  ...
  return String(best.highway || "").toLowerCase();
}
...
function synthesizeInferredSignals(maneuvers, realSignals, opts = {}) {
  const {
    ...
    maxLights = 400,
    silent = false,
    // PHASE 72: the middle path between Phase 36 and Phase 55.
    //
    // Phase 36 removed every gate to prove the render pipeline worked; Phase 55
    // switched the whole thing off after it painted countdowns on a roundabout.
    // Neither is right for daily use — brute force lies, and off leaves a city
    // with thin OSM coverage looking empty.
    //
    // This restores ONE gate: the class of the road at the maneuver. A signal
    // at a residential corner is unlikely; a signal where two secondary roads
    // meet is close to certain. It needs the corridor samples, which is exactly
    // why Phase 36 dropped it — and that dependency is now a feature. With
    // Overpass down there is no evidence, so this emits nothing rather than
    // inventing lights precisely when we know least.
  ...
  for (const mv of maneuvers) {
    ...
    // Road-class gate (Phase 72), when enabled.
    if (majorRoadsOnly) {
      const cls = nearestSampleClass(laneSamples, lat, lon);
      if (!cls || !MAJOR_CLASSES.has(cls)) {
        stats.rejectedMinorRoad = (stats.rejectedMinorRoad || 0) + 1;
        continue;
      }
    }
